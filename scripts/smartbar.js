$(function() {
	var $smartbar = $( ".bar-yellow");

	if($smartbar.length > 0) {
	    window.smartbar = $smartbar.clone();
	    $smartbar.remove();
    	newbar();
  // }
...
function newbar() {
    ...
    $("body").prepend(
     "<div class='notification notification--white'><a class='notification__info' href='#'><span class='notification__cta'>Read the ePaper edition.</span> The complete print edition, in its exact layout. Browse the print edition page by page, including stories and ads.</a><button class='notification__toggle'><svg class='icon-close'><use xlink:href='#icon-close' /></svg></button></div>");
